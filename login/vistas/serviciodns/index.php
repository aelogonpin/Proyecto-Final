...
//Para script 1º
      $conexion = mysqli_connect("192.168.48.135", "root", "Eloyto11#", "network_records") 
            ...
            $sql = "SELECT host_name, ip_address FROM dns_records;";
      ...
      <table class="table table-striped">
        ...
        <tbody>
          <?php while ($reg=mysqli_fetch_array($registros)) { ?>
            <tr>
              <td><?php echo $reg['ip_address']; ?></td>
              <td><?php echo $reg['host_name']; ?></td>
 ...
 //header("Content-Type: index.php; charset=UTF-8");
    ...
    //Generacion de las direcciones mediantes las insercción de valores
    $parametro1 = $_POST['ip'];
    $parametro2 = $_POST['nombre'];
      if (isset($_POST['script1'])) {
        // Aquí puedes poner el código que quieres ejecutar con el primer botón
    ...
    $comando = "/essentials/scriptdns.sh " . $parametro1 . " " . $parametro2;
...
<?php
// Redirige a otra página
//header("Content-Type: ../serviciodns/index.php; charset=UTF-8");


if (isset($_POST['script2'])) {
    // Aquí puedes poner el código que quieres ejecutar con el segundo botón
    ini_set('display_errors', 1);
    ini_set('display_startup_errors', 1);
    error_reporting(E_ALL);
...
$connection = ssh2_connect('192.168.48.102',22);
ssh2_auth_password($connection, 'root', 'Admin.123');
$comando = "/essentials/scriptbddef.sh";
$stream = ssh2_exec($connection, $comando);
stream_set_blocking($stream, true);
$stream_out = ssh2_fetch_stream($stream, SSH2_STREAM_STDIO);
echo "<pre>".stream_get_contents($stream_out)."</pre>";  
...
<form action="" method="post">
  <input type="text" placeholder="Nombre dominio" name="nombre">
  <input type="text" placeholder="Dirección IP" name="ip">
  <input type="submit" name="script1" value="Ejecutar Script 1">
</form>
<br><br><br><br>

<div  style="display: flex; justify-content: center; align-items: center;" >
<form action="" method="post" >
  <input type="submit" name="script2" value="Aplicar                 ">
</form>
</div>
